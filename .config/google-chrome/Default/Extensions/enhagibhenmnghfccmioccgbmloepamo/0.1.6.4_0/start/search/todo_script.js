...
window.loadToDoList = function() {
  ...
  if (localStorage.getItem("disable_todo") == "yes") {
    $("#lnk_todo").fadeOut();
    $(".todo-panel").fadeOut();
  } else {
    $("#lnk_todo").fadeIn();
  ...
  $("#disable_todo").prop("checked", localStorage.getItem("disable_todo") === "yes");
  $("#disable_todo").off("change");
  $("#disable_todo").on("change", function() {
    if ($("#disable_todo").is(":checked")) {
      localStorage.setItem("disable_todo", "yes");
  ...
  $("#newToDo").on("keypress", function(t) {
    if (t.which === 13 && $(this).val().length > 0) {
      ...
      let s = $("<li>");
      s.html(`<input type="checkbox" id="${l}" class="markDone"><label for="${l}">${a.toString()}</label><i class="dl_btn" dl-for="${l}"></i>`);
      s.css({
        display: "none"
      });
      $(".todo_list ul").append(s);
      s.fadeIn();
      $(this).val("");
      let i = {
        id: l,
